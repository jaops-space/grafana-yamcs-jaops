...
export function ArgumentField(props: {
  ...
  return (
    <Field key={`${labelPrefix ?? ''}${arg.name}`} label={labelPrefix ? `${labelPrefix} - ${arg.name}` : arg.name} description={arg.description} style={{ width: '100%', marginBottom: 0 }}>
      <>
        ...
        {error && <Alert title="Invalid argument" severity="error">{error}</Alert>}
      </>
    </Field>
  );
}
