  ...
  return (
    <>
      <Field label="Button Label" description="Button label" style={{ marginBottom: 0 }}>
        <Input type="text" disabled={loading} value={commandState?.label || ''} onChange={(e) => set('label', e.currentTarget.value)} style={{ width: '100%' }} />
      </Field>
      <Field label="Button Tooltip" description="Button tooltip" style={{ marginBottom: 0 }}>
        <Input type="text" disabled={loading} value={commandState?.tooltip || ''} onChange={(e) => set('tooltip', e.currentTarget.value)} style={{ width: '100%' }} />
      </Field>
      {includeIcon && (
      ...
      <Field label="Color" description="Button color" style={{ marginBottom: 0 }}>
        <ColorPickerInput onChange={(color: string) => set('color', color)} disabled={loading} color={commandState?.color || ''} />
      </Field>
      <Field label="Text Color" description="Text and icon color" style={{ marginBottom: 0 }}>
        <ColorPickerInput onChange={(color: string) => set('textColor', color)} disabled={loading} color={commandState?.textColor || ''} />
      </Field>
      ...
      {commandState?.shape === 'svg' && (
        <>
          ...
          <Field label="SVG Size" description="Controls how the background image is scaled." style={{ marginBottom: 0 }}>
            <Combobox
              options={[
          ...
          <Field label="SVG Position" description="Controls the position of the background image." style={{ marginBottom: 0 }}>
            <Combobox
              options={[
