    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' }}>
      <FormSection title="Variable behavior" separated={false}>
        <Field label="Variable" description="Variable to change" style={{ marginBottom: 0 }}>
          <Combobox disabled={loading} options={getTemplateSrv().getVariables().map((vr) => ({ label: vr.label || vr.name, value: vr.name }))} value={commandState?.variableToSet || ''} onChange={(e: SelectableValue<string>) => set('variableToSet', e.value)} />
        </Field>
        ...
        {commandState?.changeMode !== 'input' && (
          <Field label="Value" description="Value to use. You may write a custom value." style={{ marginBottom: 0 }}>
            <Combobox
              disabled={loading}
              options={((getTemplateSrv().getVariables().find((vr) => vr.name === commandState?.variableToSet) as VariableWithMultiSupport)?.options || []).map((option) => ({ label: option.text as string, value: option.value as string }))}
        ...
        <Field label="Display variable name" description="Show name in runtime" style={{ marginBottom: 0 }}>
          <Checkbox value={commandState?.showVariableLabel !== false} onChange={(e) => set('showVariableLabel', e.currentTarget.checked)} label="Show variable name" />
        </Field>
        <Field label="Comment" description="Optional comment" style={{ marginBottom: 0 }}>
          <Input type="text" disabled={loading} value={commandState?.comment || ''} onChange={(e) => set('comment', e.currentTarget.value)} />
        </Field>
        <Field label="Unit" description="Displayed next to the value" style={{ marginBottom: 0 }}>
          <Input type="text" disabled={loading} value={commandState?.unit || ''} placeholder="m/s, deg, ..." onChange={(e) => set('unit', e.currentTarget.value)} style={{ width: '100%' }} />
        </Field>
      </FormSection>

      {commandState?.changeMode !== 'input' && (
        <FormSection title="Button appearance">
          <ButtonStyleFields commandName={command.name} index={index} commandState={commandState} loading={loading} onOptionChange={onOptionChange} />
        </FormSection>
      )}
