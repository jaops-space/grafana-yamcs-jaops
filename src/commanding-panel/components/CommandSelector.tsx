...
export function CommandSelector(props: {
    ...
    return (
        <div
            style={{ display: 'grid', gridTemplateColumns: 'minmax(240px, 360px) 1fr', gap: '12px', alignItems: 'end' }}
        >
            ...
            <div style={{ minWidth: 0, paddingBottom: '2px' }}>
                ...
                <div style={{ opacity: 0.75, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {commandInfo?.shortDescription ||
                        commandInfo?.longDescription ||
                        'Select a command to configure its arguments.'}
                </div>
            </div>
        </div>
    );
