...
METRIC_NAMES = {
    "setup": "Simulator - stream setup time",
    "setup_per_stream": "Setup time per stream",
    "live_memory_growth_bytes": "Simulator - live memory used during run",
    "live_memory_growth_bytes_per_stream": "Live memory per stream",
    "total_allocated_bytes": "Simulator - total memory allocated during run",
    "values_read_per_sec": "Simulator - values read per second from buffers",
    "values_read_per_sec_per_stream": "Values read per second from buffers per stream",
    "values_read_fresh_pct": "Simulator - values read within the same 1s tick",
    "median_tick_runstream_busy": "Simulator - RunStream busy time per 1s tick",
    "frame_numeric_full": "Microbenchmark - time to convert numeric buffer to full frame",
    "frame_numeric_average": "Microbenchmark - time to convert numeric buffer to average frame",
    "frame_numeric_average_minmax": "Microbenchmark - time to convert numeric buffer to average/min/max frame",
    "frame_discrete": "Microbenchmark - time to convert discrete buffer to frame",
    "process_stream_10_values": "Microbenchmark - time to process 10 values into stream buffers",
}
METRIC_DETAILS = {
