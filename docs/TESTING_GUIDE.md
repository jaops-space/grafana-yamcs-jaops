...
Benchmark CI is conditional. On pull requests, add the `run/benchmarks` label to run the benchmark jobs and create or update the benchmark PR comment with all plots. Unlabeled PR open/sync events do not start the benchmark workflow, so benchmark jobs do not appear as skipped checks. Pushes to `main` run the benchmark jobs without PR-base comparison or PR comments. Warn thresholds keep CI green; fail thresholds fail the benchmark job.
